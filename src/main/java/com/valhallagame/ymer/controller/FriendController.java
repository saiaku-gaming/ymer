...
public class FriendController {
	...
	public ResponseEntity<?> sendCharacterInvite(@RequestAttribute("username") String username,
		...
		return JS.message(FriendServiceClient.get().sendCharacterInvite(username, input.getCharacterName()));
	}

	@RequestMapping(path = "/accept-person-invite", method = RequestMethod.POST)
	public ResponseEntity<?> acceptPersonInvite(@RequestAttribute("username") String username, @RequestBody UsernameParameter input)
			throws IOException {
	...
	public ResponseEntity<?> acceptCharacterInvite(@RequestAttribute("username") String username, @RequestBody CharacterNameParameter input)
			throws IOException {
		return JS.message(FriendServiceClient.get().acceptCharacterInvite(username, input.getCharacterName()));
	}
	...
	public ResponseEntity<?> declineCharacterInvite(@RequestAttribute("username") String username,
			@RequestBody CharacterNameParameter input) throws IOException {
		return JS.message(FriendServiceClient.get().declineCharacterInvite(username, input.getCharacterName()));
	}
	...
	public ResponseEntity<?> removeCharacterFriend(@RequestAttribute("username") String username,
			@RequestBody CharacterNameParameter input) throws IOException {
		return JS.message(FriendServiceClient.get().removeCharacterFriend(username, input.getCharacterName()));
	}
}
